...
def build_market_pages(query, items, updated_at):
    ...
    for start in range(0, total, per_page):
        ...
        if updated_text:
            header.set_footer(
                text=f"머장봇 · 모비라이프 거래소 · 갱신 {updated_text}"
            )
        else:
            header.set_footer(text="머장봇 · 모비라이프 거래소")
...
class MarketPaginationView(discord.ui.View):
    """시세 검색 결과를 같은 메시지에서 이전/다음 버튼으로 넘긴다."""
...
async def market_price_command(
    ...
    try:
        ...
        if len(pages) == 1:
            ...
            view = MarketPaginationView(pages)
