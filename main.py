...
def install_abyss_subscriptions(client):
    ...
    original_send_abyss_alert = module.send_abyss_alert
    ...
    async def send_abyss_alert_with_mentions(
        ...
        if not subscriber_ids:
            return await original_send_abyss_alert(
                channel,
                spawn_utc,
                minutes_before,
                estimated,
        ...
        mention_chunks = build_mention_chunks(subscriber_ids)
        ...
        class MentioningChannel:
            def __init__(self, real_channel):
                self._real_channel = real_channel
            ...
            async def send(self, *args, **kwargs):
                for chunk in mention_chunks[:-1]:
                    await self._real_channel.send(
                        content=chunk,
                        allowed_mentions=allowed_mentions,
                    )

                kwargs["content"] = mention_chunks[-1]
                kwargs["allowed_mentions"] = allowed_mentions
                return await self._real_channel.send(*args, **kwargs)

        result = await original_send_abyss_alert(
            MentioningChannel(channel),
            spawn_utc,
            minutes_before,
            estimated,
        )
...
# 기존 본체가 client.run()을 호출하기 직전에 확장 기능을 붙인다.
_original_client_run = discord.Client.run


def _run_with_extensions(self, token, *args, **kwargs):
    ...
    install_abyss_subscriptions(self)

    return _original_client_run(self, token, *args, **kwargs)


discord.Client.run = _run_with_extensions
