...
def cleanup_old_abyss_alerts():
    cutoff = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat()
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM abyss_alerts WHERE spawn_time < ?", (cutoff,))
    conn.commit()
    conn.close()
...
async def get_abyss_records(force_key_refresh=False):
    ...
    async with aiohttp.ClientSession(headers=headers, timeout=timeout) as session:
        if moblife_key is None or force_key_refresh:
            moblife_key = await discover_moblife_key(session)
            print("[어비스] 모비라이프 데이터 키 확인 완료")
    ...
    try:
        ...
        # 사이트가 배포되며 키가 바뀐 경우 한 번 새로 찾아 재시도
